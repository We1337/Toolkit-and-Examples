...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
    ...
    foreach ($student_ids as $key => $student_id) {
        ...
        $update_values = array_merge([$student_name], $student_marks, [$student_id]);
...
if ($result) {
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Generate input fields for marks starting from week1 to week16
            for ($i = 1; $i <= 16; $i++) {
                $week = 'week' . $i;
                echo "<td><input type='text' name='marks[{$row['student_id']}][$i]' value='{$row[$week]}' style='width: 40px;'></td>";
            }
